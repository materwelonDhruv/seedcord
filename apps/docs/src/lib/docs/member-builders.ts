...
export const resolveHeaderSignature = async (node: DocNode, context: FormatContext): Promise<CodeRepresentation> => {
    if (node.header) {
...
const stripDuplicateDescription = (
...
const deriveSharedDocumentation = (
    nodeComment: FormattedComment,
    description: CommentParagraph,
...
const buildSignatureDetails = async ({
    node,
    context,
    signatureComments,
    description,
...
export const buildTypeParameterSummaries = async (
...
export const buildMemberSummary = async (node: DocNode, context: FormatContext): Promise<EntityMemberSummary> => {
