            ...
            <DropdownMenu.Root open={open} onOpenChange={setOpen} modal={false}>
                <DropdownMenu.Trigger asChild>
                    <button
                        ...
                        aria-expanded={open}
                        className={cn(
                            'flex w-full items-center justify-between rounded-xl border border-border/80 bg-[color-mix(in_srgb,var(--bg)_96%,var(--surface)_4%)] px-3 py-2 text-sm font-medium text-(--text) shadow-soft transition duration-0',
                ...
                <DropdownMenu.Portal>
                    <DropdownMenu.Content
                        align="start"
                        sideOffset={8}
                        className="z-70 w-(--radix-dropdown-menu-trigger-width) min-w-[220px] overflow-hidden rounded-xl border border-border/80 bg-[color-mix(in_srgb,var(--bg)_96%,#070917_4%)] p-1 shadow-[0_18px_36px_-16px_color-mix(in_srgb,var(--text)_32%,transparent)] transition-colors duration-200"
                    >
